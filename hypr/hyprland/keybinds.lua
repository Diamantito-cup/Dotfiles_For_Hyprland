...
create_bind(
	vars.kbScreenshot,
	hl.dsp.exec_cmd(
		'sh -c \'f=$HOME/Imágenes/Capturas\\ de\\ pantalla/captura_$(date +%Y%m%d_%H%M%S).png; grim "$f" && wl-copy < "$f" && notify-send "Captura realizada" "Guardada y copiada al portapapeles" -i "$f"\''
	)
)
create_bind(vars.kbScreenshotFreeze, hl.dsp.exec_cmd("flameshot gui"))
create_bind(vars.kbScreenshotRegion, hl.dsp.exec_cmd("flameshot gui"))
